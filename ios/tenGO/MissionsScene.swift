...
class MissionsScene: SKScene {
    ...
    private func addMissionRow(_ mission: MissionManager.DisplayMission, atY y: CGFloat, height: CGFloat) {
        ...
        title.preferredMaxLayoutWidth = width - 130
        title.position = CGPoint(x: -width / 2 + 20, y: 18)
        row.addChild(title)

        // Barre de progression.
        let barW: CGFloat = width - 40
        let barH: CGFloat = 8
        ...
        fill.position = CGPoint(x: -barW / 2 + fillW / 2, y: -14)
        row.addChild(fill)
        ...
        progressLabel.position = CGPoint(x: -width / 2 + 20, y: -34)
        row.addChild(progressLabel)

        // Récompense / bouton de réclamation, aligné à droite.
        if mission.claimed {
            ...
            check.position = CGPoint(x: width / 2 - 44, y: 0)
            row.addChild(check)
        } else if mission.isCompleted {
            let claimBtn = SKNode()
            claimBtn.name = "claim_\(def.id)"
            claimBtn.position = CGPoint(x: width / 2 - 62, y: 0)
            row.addChild(claimBtn)

            let claimBg = SKShapeNode(rectOf: CGSize(width: 96, height: 44), cornerRadius: 22)
            ...
            coin.position = CGPoint(x: width / 2 - 78, y: 0)
            row.addChild(coin)
            ...
            rewardLabel.position = CGPoint(x: width / 2 - 62, y: 0)
        ...
        if def.isSuper {
            ...
            badge.position = CGPoint(x: -width / 2 + 20, y: height / 2 - 14)
            row.addChild(badge)
        }
    }
